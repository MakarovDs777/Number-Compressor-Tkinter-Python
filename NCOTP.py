...
# - Результат — многозначные разделены пробелами,
#   однозначные «прилипают» к предыдущему слову:
#   "252Q1 253Q5 128Q3"
#
# Разжатие:
# - "252Q1 253Q5 128Q3" -> "252 1 253 5 128 3"
# ============================================================
...
def compress_to_compact(numbers: list):
    """
    [252, 1, 253, 5, 128, 3] -> "252Q1 253Q5 128Q3"

    Многозначные остаются строками и разделяются пробелами.
    Однозначные заменяются на Q-код и «приклеиваются» к предыдущему элементу
    (пробел перед ними убирается).
    """
    parts = []
    for i, n in enumerate(numbers):
        if 0 <= n <= 9:
            # Однозначное: убираем пробел перед ним
            if parts and parts[-1] == " ":
                parts.pop()  # удаляем пробел
            parts.append(DIGIT_TO_Q[n])
            # Если следующее число — многозначное, после Q-кода ставим пробел
            if i + 1 < len(numbers) and not (0 <= numbers[i + 1] <= 9):
                parts.append(" ")
        else:
            parts.append(str(n))
            # После многозначного ставим пробел (даже если дальше Q-код —
            # пробел уберётся при обработке следующего однозначного)
            if i + 1 < len(numbers):
                parts.append(" ")
...
def decompact_to_numbers(compact: str):
    """
    "252Q1 253Q5 128Q3" -> [252, 1, 253, 5, 128, 3]
    Также обрабатывает слитный формат "252Q1253Q5128Q3".
    """
    nums = []
    i = 0
    n = len(compact)
...
def open_image_from_text():
    ...
    stripped = txt.strip()
    if "Q" in stripped and not any(ch == " " for ch in stripped if ch.isdigit() is False and ch != "Q"):
        # Похоже на сжатый формат без пробелов — разжимаем
        pass  # decompact_to_numbers обрабатывает оба варианта

    if "Q" in stripped:
        # Пробуем разжать как Q-формат
        try:
...
tk.Label(
    ...
    text=(
        ...
        "«Сжать» — однозначные (0–9) → Q0..Q9, пробел перед ними убирается,\n"
        "  многозначные остаются и разделяются пробелами. Пример: 252 1 5 → 252Q1 Q5\n"
        "«Разжать» — обратно: 252Q1 Q5 → 252 1 5\n"
